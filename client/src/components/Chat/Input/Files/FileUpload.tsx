...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    reader.onload = (e) => {
      ...
      if (validator) {
        setStatus('success');
        setStatusColor('text-pink-500 dark:text-pink-500');
      }
  ...
  return (
    <label
      ...
      className={cn(
        'mr-1 flex h-auto cursor-pointer items-center rounded bg-transparent px-2 py-1 text-xs font-normal transition-colors hover:bg-blue-700 hover:text-blue-100 dark:bg-transparent dark:text-gray-20 dark:hover:bg-pink-700 dark:hover:text-pink-20', // Changed color number: hover:bg-gray-100 -> hover:bg-blue-700, hover:text-gree-600 -> hover:text-blue-20, dark:hover:text-pink-500 -> dark:hover:text-pink-200
        statusColor,
        containerClassName,
      )}
    >
